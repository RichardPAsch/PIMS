...
namespace PIMS.Web.Api.Controllers
{
	...
	public class AccountController : ApiController
	{
		...
		public async Task<IHttpActionResult> RegisterAsync([FromBody] RegistrationModel registrationData) {
			...
			// Persist data to : AspNetUsers & ApplicationUser tables.
			var identityResult = await UserMgr.CreateAsync(user, registrationData.Password);  
			
			// Persist skeletal data for Investor table. 
			var investorIsSaved = PersistInvestorForAccount(user.Id, user.UserName);
		
			var errorMsg = string.Empty;
			if (identityResult.Succeeded && investorIsSaved)
			{
		...
		private bool PersistInvestorForAccount(string investorId, string userName)
		{
			var connString = NHibernateConfiguration.GetConnectionString();
			var sessionFactory = NHibernateConfiguration.CreateSessionFactory(connString);
			var investorCtrl = new InvestorController(new InvestorRepository(sessionFactory));

			// TODO: Deferred for possible later enhancements, e.g., new 'Investor Profile' form to be filled during registration, or
			// TODO: at a later time?
			// Minimal data initially supplied for 'Investor' table.
			var investor = new Investor {
				InvestorId = Guid.NewGuid(),
				AspNetUsersId = Guid.Parse(investorId),
				LastName = "pending",
				FirstName = "pending",
				BirthDay = "pending",
				Address1 = "pending",
			...
			var investorSaveResult = investorCtrl.CreateNewInvestor(investor);
		    sessionFactory.Dispose();

		    return investorSaveResult.Status.ToString() == "RanToCompletion";
		}
